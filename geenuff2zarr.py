...
def main(args):
    ...
    if args.add_additional:
        match_existing = True
        zarr_group = '/alternative/' + args.add_additional + '/'
    else:
        match_existing = False
        zarr_group = '/data/'

    write_by = round(args.write_by / args.subsequence_length) * args.subsequence_length
    controller = HelixerExportController(args.input_db_path, args.zarr_output_path, match_existing=match_existing,
                                         zarr_group=zarr_group)
    controller.export(chunk_size=args.subsequence_length, write_by=write_by, modes=modes,
...
if __name__ == '__main__':
    ...
    pp.io_group.add_argument('--add-additional', type=str,
                             help='Outputs the datasets under alternatives/{add-additional}/ (and checks sort order '
    ...
    pp.data_group.add_argument('--modes', type=str,
                               help='Either "all" (default), or a comma separated list with desired members of the '

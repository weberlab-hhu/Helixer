...
- [fasta2zarr.py](fasta2zarr.py): conversion of the DNA sequence to numerical matrices
...
HybridModel.py --load-model-path $HOME/.local/share/Helixer/models/land_plant/land_plant_v0.3_a_0080.h5 \
     --test-data Arabidopsis_lyrata.zarr --overlap --val-test-batch-size 32 -v --predict-phase

# order of input parameters:
# helixer_post_bin <genome.zarr> <predictions.zarr> <window_size> <edge_threshold> <peak_threshold> <min_coding_length> <output.gff3>
helixer_post_bin Arabidopsis_lyrata.zarr predictions.zarr 100 0.1 0.8 60 Arabidopsis_lyrata_chromosome8_helixer.gff3
```
...
|:------------------|:-------------------|:--------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| genome.zarr       | 1                  | /       | Zarr file containing the genome assembly; output of `fasta2zarr.py`                                                                                                                                   |
| predictions.zarr  | 2                  | /       | Zarr file containing the predictions from Helixer; output of `HybridModel.py`                                                                                                                         |
